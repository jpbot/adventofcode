...
## 797908 TOO LOW (missed fixing the data to match the computer state)
...
# add more instructions to the computer, this is always 4 (1 opcode + 3
...
# 	print("PART A: INPUTDATA: {}".format(localdata))
	
	while(pos < len(localdata)):
		if localdata[pos] == OP_ADD:
			localdata[localdata[pos + 3]] = (
				localdata[localdata[pos + 1]]
				+ localdata[localdata[pos + 2]])		
		elif localdata[pos] == OP_MULT:
			localdata[localdata[pos + 3]] = (
				localdata[localdata[pos + 1]]
				* localdata[localdata[pos + 2]])		
		elif localdata[pos] == OP_TERM:
# 			print("PART A: RESULTDATA: {}".format(localdata))
...
def partb(data):
	...
	for noun in range(99):
		for verb in range(99):
			...
			r,err = parta(intcode)
			if(err):
				exit_err("PARTB PART A error. ({},{})".format(noun, verb), 1)
			if(r == goal):			
...
intcode = []
read_data(intcode)
print("DEBUG: intcode length: {}".format(len(intcode)))
# r,err = parta(testdata)
# if(err):
# 	exit_err("PART A error.", 1)

# FIX DATA (for part one)
## Once you have a working computer, the first step is to restore the gravity assist
## program (your puzzle input) to the "1202 program alarm" state it had just before the
## last computer caught fire. To do this, before running the program, replace position 1
## with the value 12 and replace position 2 with the value 2. What value is left at
...
r,err = partb(intcode)
if(err):
	exit_err("PART B error.", 1)
print("PARTB: {}".format(r))
