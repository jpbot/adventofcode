...
size_t light_count(int *lights, size_t count);
void light_set(int *lights, char set, int x, int y, int max_x);
char light_get(int *lights, int x, int y, int max_x);
void light_toggle(int *lights, int x, int y, int max_x);
void light_set_range(int *lights, char set, int x1, int y1, int x2, int y2, int max_x);
void light_toggle_range(int *lights, int x1, int y1, int x2, int y2, int max_x);
...
int main(int argc, char **argv){
   ...
   memset(lights, 0, MAXLIGHTS * sizeof(int));
   ...
   logf(ERRINFO, "%lu total brightness of lights after following directions from input file: %s", light_count(lights, MAXLIGHTS), inputfile);
...
void light_set(int *lights, char set, int x, int y, int max_x){
   int *light_under_control = lights+x*max_x+y;

   *light_under_control += set;
   if(*light_under_control < 0)   *light_under_control = 0;
}
...
size_t light_count(int *lights, size_t count){
   size_t light_ct = 0;
   int *last_light = lights + count - 1;

   for(; lights < last_light + 1;){
      light_ct += (size_t) *lights++;
   }
