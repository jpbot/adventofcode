...
int main(int argc, char **argv){
   ...
   totalarea = scan_file(infile, &wrap_area);
   logf(ERRINFO, "Please order %d square feet of wrapping paper", totalarea);
   ...
   while(ch != EOF){
      if(ch == '\n'){
         *bufpos = 0;
         ...
         logf(ERRDEBUG, "PARSE: %d x %d x %d", x, y, z);
         logf(ERRDEBUG, "AREA:  %d", wrap_area(x, y, z));
         logf(ERRDEBUG, "TOTAL: %d", retval);
      }else{
